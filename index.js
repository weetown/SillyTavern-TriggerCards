...
const createNametag = (nameText, position) => {
    const tag = document.createElement('div');
    tag.classList.add('sttc--nametag');
    tag.classList.add(position === 'above' ? 'sttc--nametag-above' : 'sttc--nametag-below');
    tag.textContent = nameText;
    return tag;
};

const applyDragReorderState = () => {
    if (!trayRow) return;
    for (const wrap of trayRow.querySelectorAll('.sttc--wrapper')) {
        wrap.draggable = Boolean(settings.enableDragReorder);
    }
};

const applyNametagLayoutToExistingCards = () => {
    if (!trayRow) return;
    for (const wrap of trayRow.querySelectorAll('.sttc--wrapper')) {
        for (const oldTag of wrap.querySelectorAll('.sttc--nametag')) oldTag.remove();
        if (!settings.showNametags) continue;

        const img = wrap.querySelector('.sttc--img');
        const name = img?.getAttribute('data-character')?.split('::')[0] ?? '';
        const tag = createNametag(name, settings.nametagPosition);
        if (settings.nametagPosition === 'above') {
...
const applyLiveSettings = (changedKeys = []) => {
    if (!root) return;
    applyRootStyleSettings();

    if (changedKeys.length === 0) return;
    if (changedKeys.includes('enableDragReorder')) applyDragReorderState();
    if (changedKeys.includes('showNametags') || changedKeys.includes('nametagPosition')) {
        applyNametagLayoutToExistingCards();
    }
};
...
const loadSettings = ()=>{
    ...
    settings.onLiveUpdate = (changedKeys)=>applyLiveSettings(changedKeys);
        ...
        for (const name of added) {
            ...
            const wrap = document.createElement('div'); {
                ...
                if (settings.showNametags && settings.nametagPosition === 'above') {
                    wrap.append(createNametag(namePart, 'above'));
                }
                ...
                if (settings.showNametags && settings.nametagPosition !== 'above') {
                    wrap.append(createNametag(namePart, 'below'));
                }
